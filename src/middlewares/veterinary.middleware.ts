...
//     if (!record) {
//       return res.status(404).send({ error: `Veterinary ${id} not found` });
//     }
...
//   async validateVeterinaryExistsByBody(req: express.Request, res: express.Response, next: express.NextFunction) {
//     const { veterinaryId } = req.body;
//     if (!veterinaryId) {
//       return res.status(500).send({ error: `Must be send a veterinaryId property on body` });
//     }
//     const record = await veterinaryService.getById(veterinaryId);
//     if (!record) {
//       return res.status(404).send({ error: `Veterinary ${veterinaryId} not found` });
//     }
//     next();
//   }
// }
